...
import { GUARDIAN_MODEL, GUARDIAN_NAME, PREDECESSOR_GIVEN, PREDECESSOR_NAME } from '../../world/company.js'
...
export const rawChapter02Scenes = {
  ...
  server_panel_chat: {
    ...
    lines: [
      ...
      { char: 'groomy', text: '명령 이름은 "BATTERY_COMPAT_CHECK"예요.' },
      { char: 'groomy', text: '사무용 장비는 아닌 것 같아요.' },
      { char: 'groomy', text: '아마.' },
  ...
  records_vn: {
    ...
    lines: [
      ...
      { char: 'groomy', text: '그래서 지우기 어려운 것 같아요.', portrait: { charId: 'unknown', baseId: ASSETS.portrait.unknownSmile, position: 'right' }, portraitAssetId: ASSETS.portrait.unknownSmile },
      { char: 'groomy', text: '지우려면 누군가 살아 있어야 하고요.', portrait: { charId: 'unknown', baseId: ASSETS.portrait.unknownSmile, position: 'right' }, portraitAssetId: ASSETS.portrait.unknownSmile },
      { char: 'groomy', text: '아마.', portrait: { charId: 'unknown', baseId: ASSETS.portrait.unknownSmile, position: 'right' }, portraitAssetId: ASSETS.portrait.unknownSmile },
    ],
    next: 'records_rpg',
  ...
  vault_terminal_chat: {
    ...
    lines: [
      { char: 'system', text: `감사 원장에 "${PREDECESSOR_GIVEN} / 임시매핑 / 42초차" 항목이 반복된다.`, isNarration: true, important: true },
      { char: 'groomy', text: '회사는 이걸 버그라고 불러요.' },
      { char: 'groomy', text: '우리는 패턴이라고 부를 수 있을 것 같아요.' },
      { char: 'groomy', text: '아마.' },
    ],
    choices: [
      {
  ...
  analyst_chat: {
    ...
    lines: [
      ...
      { char: 'iseol', text: `한쪽은 당신, 다른 한쪽은 ${PREDECESSOR_NAME}의 마지막 접속 기록이에요.` },
      { char: 'groomy', text: '타임스탬프가 이상해요.' },
      { char: 'groomy', text: '같은 사번인데 42초 차이로 서로 다른 문을 통과한 것 같아요.' },
      { char: 'groomy', text: '아마.' },
    ],
    choices: [
      {
      ...
      {
        text: '개인 단말 충전·주문 로그를 확인한다.',
        next: 'ara_battery_check',
        requirements: [{ type: 'unlessFlag', flag: 'araInvestigated' }],
      },
    ],
  },
  ara_battery_check: {
    id: 'chapter-02.ara_battery_check',
    chapterId: 'chapter-02',
    localId: 'ara_battery_check',
    mode: SceneModes.CHAT,
    emotion: 'warning',
    important: true,
    returnTo: 'floor5_rpg',
    lines: [
      { char: 'system', text: '분석가 터미널 옆 폐쇄 채널에 충전 주문서가 한 건 남아 있다.', isNarration: true },
      {
        char: 'system',
        text: `수취: ${GUARDIAN_NAME} · ${GUARDIAN_MODEL} · 잔여 14%. 발주: 회사 승인 대기.`,
        isNarration: true,
        important: true,
      },
      { char: 'groomy', text: '...그건 당신 집으로 가는 줄 알았어요.' },
      { char: 'groomy', text: '회사가 먼저 본 것 같아요.' },
      { char: 'groomy', text: '아마.' },
    ],
    choices: [
      {
        text: '기록을 닫고 조사로 돌아간다.',
        next: 'floor5_rpg',
        effects: [
          { type: EffectTypes.ADD_FLAG, flag: 'araInvestigated' },
          { type: EffectTypes.ADD_ITEM, item: 'araGuardianChargeLog' },
          batteryPressure(1),
          evidence(1),
        ],
      },
  ...
  locker_chat: {
    ...
    lines: [
      ...
      { char: 'groomy', text: '이 규격은 사무실 장비용이 아닌 것 같아요.' },
      { char: 'groomy', text: '누군가 살아 있어야 하는 장치 쪽에 가까워요.' },
      { char: 'groomy', text: '아마.' },
    ],
    choices: [
      {
  ...
  escalation_vn: {
    ...
    lines: [
      ...
      { char: 'groomy', text: '남길지.', portrait: { charId: 'unknown', baseId: ASSETS.portrait.unknownSmile, position: 'center' }, portraitAssetId: ASSETS.portrait.unknownSmile },
      { char: 'groomy', text: '지울지.', portrait: { charId: 'unknown', baseId: ASSETS.portrait.unknownSmile, position: 'center' }, portraitAssetId: ASSETS.portrait.unknownSmile },
      { char: 'groomy', text: '아니면 덮을지.', portrait: { charId: 'unknown', baseId: ASSETS.portrait.unknownSmile, position: 'center' }, portraitAssetId: ASSETS.portrait.unknownSmile },
      { char: 'groomy', text: '다음 방에서 결정하는 것 같아요.', portrait: { charId: 'unknown', baseId: ASSETS.portrait.unknownSmile, position: 'center' }, portraitAssetId: ASSETS.portrait.unknownSmile },
      { char: 'groomy', text: '아마.', portrait: { charId: 'unknown', baseId: ASSETS.portrait.unknownSmile, position: 'center' }, portraitAssetId: ASSETS.portrait.unknownSmile },
    ],
    returnTo: 'floor5_rpg',
  ...
  mirror_clue_vn: {
    ...
    lines: [
      {
        char: 'system',
        text: `깨진 모니터마다 서로 다른 시간의 ${PREDECESSOR_NAME}이 비친다.`,
        isNarration: true,
      },
      {
        char: 'system',
  ...
  groomy_debrief: {
    ...
    mode: SceneModes.CHAT,
    lines: [
      { char: 'groomy', text: '2장은 여기까지예요.' },
      { char: 'groomy', text: '당신이 남긴 기록은 제 메모리에도 남는 것 같아요.' },
      { char: 'groomy', text: '배터리.' },
      { char: 'groomy', text: '사번.' },
      { char: 'groomy', text: '순찰 공백.' },
      { char: 'groomy', text: '셋 중 둘만 맞아도 회사는 움직이는 것 같아요.' },
      { char: 'groomy', text: '아마.' },
      { char: 'groomy', text: '다음 장에서는 "진실"과 "보호자" 중 무엇을 먼저 고를지 정해질 것 같아요.' },
    ],
    next: 'floor3_door_approach',
    effects: [{ type: EffectTypes.ADD_FLAG, flag: 'ch2_completedInvestigation' }],
  },
  floor3_door_approach: {
    id: 'chapter-02.floor3_door_approach',
    chapterId: 'chapter-02',
    localId: 'floor3_door_approach',
    mode: SceneModes.VN,
    emotion: 'warning',
    important: true,
    backgroundAssetId: 'bg_stairwell_floor3',
    vnStage: {
      bgId: 'bg_stairwell_floor3',
      overlayId: ASSETS.overlay.glitchSoft,
      characters: [],
    },
    lines: [
      { char: 'system', text: '배터리 로그를 따라 3층 자료실로 향한다.', isNarration: true, important: true },
      { char: 'system', text: '문 앞. 잠겨 있지 않다.', isNarration: true, important: true },
      { char: 'player', text: '…?' },
    ],
    next: 'chapter_end',
    effects: [{ type: EffectTypes.ADD_FLAG, flag: 'ch2_reachedFloor3Door' }],
  },
  chapter_end: {
