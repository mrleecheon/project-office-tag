...
export function resolveProjectGroomyEnding(state) {
  const mysterySolved = state.flags.includes('truthExposed') ||
    (state.scores.mysteryEvidence ?? 0) >= PROJECT_GROOMY_ENDING_THRESHOLDS.mysterySolved
  const closeWithGroomy = state.flags.includes('groomyStayedClose') ||
    (state.scores.groomyAffinity ?? 0) >= PROJECT_GROOMY_ENDING_THRESHOLDS.closeGroomy

  if (mysterySolved && closeWithGroomy) return PROJECT_GROOMY_ENDINGS.true
  if (!mysterySolved && closeWithGroomy) return PROJECT_GROOMY_ENDINGS.normal
  if (mysterySolved && !closeWithGroomy) return PROJECT_GROOMY_ENDINGS.badA
  return PROJECT_GROOMY_ENDINGS.badB
}
...
export function resolveChapter02Ending(state) {
  ...
  if (battery >= CHAPTER_02_ENDING_THRESHOLDS.battery && evidence >= CHAPTER_02_ENDING_THRESHOLDS.investigation) {
    return CHAPTER_02_ENDINGS.coverup
  }
  if (corporate >= CHAPTER_02_ENDING_THRESHOLDS.corporate && evidence >= CHAPTER_02_ENDING_THRESHOLDS.investigation) {
...
export function resolveChapterClearCopy(chapterId, state) {
  ...
  if (chapterId === 'chapter-01') {
    return {
      ...
      title: '첫 번째 날',
      body: `${state.nickname}님은 ${state.scores.mysteryEvidence ?? 0}개의 의심스러운 기록을 남겼습니다.`,
      sub: `그루미 관계도: ${state.scores.groomyAffinity ?? 0}`,
    }
  }
  ...
  return {
    ...
    sub: resolveProjectGroomyEnding(state).title,
  }
}
